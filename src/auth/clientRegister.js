...
export default function ClientRegister() {
    ...
    async function handleSubmit(e){
        ...
        const res = await axios.get('http://localhost:5000/api/get-clients');
        setClientData(res.data);

        console.log(clientData);

        for(let i=0; i<clientData.length; i++){
            if(clientData[i].client_id === formData.clientId){
        ...
        for(let i=0; i<clientData.length; i++){
            if(clientData[i].email === formData.email){
                toast.error("Mail is already registered")
                return;
            }
        }
